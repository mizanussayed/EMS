...
export default function FeeConfiguration({ onConfigUpdate }: FeeConfigProps) {
  const api = useApi();
  ...
  const fetchStructures = useCallback(async () => {
    try {
      setLoading(true);
      // Get all fees and group by class and month to show configured structures
  ...
  useEffect(() => {
    fetchClasses();
    fetchStructures();
  }, [fetchClasses, fetchStructures]);

  const formFields: FormField[] = [
    { name: 'className', label: 'Class', type: 'select', options: classes.map(c => ({label: c, value: c})), required: true },
    { name: 'month', label: 'Month', type: 'select', options: months.map(m => ({label: m, value: m})), required: true },
  ...
  const handleSubmit = async (data: any) => {
    try {
      if (modalMode === 'add') {
        // Note: This would need a dedicated endpoint to create fee structures
        // For now, we'll create individual fees for each student in the class
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-gray-900 font-bold text-xl flex items-center gap-2">
            <Settings className="w-5 h-5 text-[#2D6CDF]" />
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        {structures.map((structure, idx) => (
          <div key={idx} className="bg-white rounded-xl border border-gray-100 p-5 hover:shadow-lg transition-shadow">
            ...
            {structure.description && (
              <p className="text-xs text-gray-400 mt-3">{structure.description}</p>
            )}
          </div>
        ))}
      </div>
