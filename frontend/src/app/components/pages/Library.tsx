      ...
      accessor: (b) => (
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <div className="text-xs text-gray-400">by {b.author}</div>
          </div>
        </div>
      )
    },
    ...
    { 
      ...
      accessor: (b) => (
        <div className="flex flex-col">
          <span className="font-bold text-gray-900">{b.availableQuantity} / {b.quantity}</span>
          <span className="text-[10px] text-gray-400 uppercase tracking-widest font-black">Available</span>
        </div>
      )
    },
    { header: 'Location', accessor: (b) => <span className="font-bold text-gray-700">{b.rackNumber}</span> }
  ];

  const stats = [
    { label: 'Total Books', value: books.reduce((acc, b) => acc + b.quantity, 0) },
    { label: 'Active Loans', value: issuedBooks.length, color: 'text-orange-600' },
    { label: 'Available', value: books.reduce((acc, b) => acc + b.availableQuantity, 0), color: 'text-green-600' },
    { label: 'Categories', value: new Set(books.map(b => b.category)).size },
  ];
    ...
    <div className="p-6">
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-8">
        <div className="lg:col-span-3">
          <GenericTable
            ...
            addLabel="Add Book"
            onView={(b) => {
              setIssueFormData({...issueFormData, bookId: String(b.id)});
      ...
      >
        <GenericForm
          ...
          initialData={{ quantity: 1, category: 'Science', rackNumber: 'A-1' }}
