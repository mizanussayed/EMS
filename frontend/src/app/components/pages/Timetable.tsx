...
import { Clock, Edit, User, MapPin, Plus, Trash2, X } from 'lucide-react';
...
const getTimetableFormFields = (classOptions: string[]): FormField[] => [
  ...
  { name: 'endTime', label: 'End Time', type: 'select', options: timeSlots.map(t => ({label: t, value: t})), required: true },
];
  ...
  return (
    <div className="p-6">
                              ...
                              {auth?.role === 'admin' && (
                                <div className="flex gap-1 opacity-0 group-hover/card:opacity-100 transition-opacity">
                                  <button
                                    ...
                                    className="p-1.5 hover:bg-blue-100 rounded-lg transition-colors text-[#2D6CDF]"
                                    title="Edit"
                                  >
      ...
      <Modal
        ...
        title={modalMode === 'add' ? 'New Schedule Entry' : 'Edit Schedule Entry'}
      >
        <GenericForm
          fields={getTimetableFormFields(classes)}
          initialData={selectedEntry || { dayOfWeek: 'Monday', startTime: '08:00', endTime: '09:00' }}
          onSubmit={handleSubmit}
          onCancel={() => setShowModal(false)}
          submitLabel={modalMode === 'add' ? 'Create Entry' : 'Update Entry'}
        />
      </Modal>
