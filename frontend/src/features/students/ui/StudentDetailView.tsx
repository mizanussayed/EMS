...
import { useStudent, useAttendanceByClass } from '../hooks/useStudents';
...
export default function StudentDetailView() {
  ...
  const { auth } = useAuth();
  const { data: student, isLoading: loading } = useStudent(id);
  ...
  const attendanceQuery = useAttendanceByClass(student?.className);

  useEffect(() => {
    setAttendanceLoading(Boolean(attendanceQuery.isFetching));
  }, [attendanceQuery.isFetching]);

  useEffect(() => {
    if (!attendanceQuery.data || !student) {
      setAttendance([]);
      return;
    }

    const filtered = (attendanceQuery.data as any[]).filter((record: any) => {
  ...
  }, [attendanceQuery.data, student, selectedMonth, selectedYear]);
          ...
          <div className="flex flex-col md:flex-row gap-8">
            <div className="flex flex-col items-center gap-3 min-w-[140px]">
              ...
              <div className="text-center">
                ...
                <span className={`inline-block mt-2 px-3 py-1 rounded-full text-[10px] font-black uppercase tracking-widest ${student.isActive ? 'bg-green-50 text-green-600 border border-green-100' : 'bg-red-50 text-red-600 border border-red-100'}`}>{student.isActive ? 'Active' : 'Inactive'}</span>
              </div>
            </div>
            ...
            <div className="flex-1 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-x-10 gap-y-4">
              {[
                ...
                { label: 'Section', value: student.sectionName ?? '—', badge: 'purple' },
      ...
      <div className="bg-white rounded-3xl border border-gray-100 shadow-sm overflow-hidden">
        ...
        <div className="p-8">
          ...
          <Section title="Academic Info" icon={BookOpen}>
            ...
            </Section><Section title="Parent / Guardian" icon={User}>
...
function Section({ title, icon: Icon, children }: { title: string; icon: typeof User; children: React.ReactNode }) {
  return <div className="bg-gray-50 rounded-2xl border border-gray-100 p-6">
    <div className="flex items-center gap-2.5 mb-5">
      <div className="w-8 h-8 rounded-xl bg-[#2D6CDF]/10 flex items-center justify-center">
      <Icon className="w-4 h-4 text-[#2D6CDF]" /></div>
...
function InfoRow({ label, value }: { label: string; value: string }) {
  return <div className="flex justify-between items-start py-2.5 border-b border-gray-100 last:border-0">
